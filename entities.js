...
// --- VISUAL CLASSES ---
// Assume 'projectiles' is a globally defined array.
const PROJECTILE_SPEED = 400; // px/sec
const PROJECTILE_RADIUS = 8;
const PROJECTILE_LIFE = 1.0; // seconds
...
class Projectile {
     ...
     */
    constructor(startX, startY, targetX, targetY, color = '#bb86fc') {
        this.x = startX;
        this.y = startY;
    ...
    update(dt) {
        this.x += this.vx * dt;
        this.y += this.vy * dt;
        this.distanceTraveled += PROJECTILE_SPEED * dt;

        // Check for impact or life expiration
        if (this.distanceTraveled >= this.distanceToTarget || this.life <= 0) {
            this.onImpact();
    ...
    /**
     * Handles projectile impact, spawning micro-particles.
     */
    onImpact() {
        // Spawn micro-particles at impact location
        for (let i = 0; i < 10; i++) {
    ...
    /**
     * Draws the projectile with SOTA rendering techniques.
    ...
    draw(ctx) {
        ...
        ctx.globalCompositeOperation = 'lighter'; // Blend mode for glow
...
/**
 * Spawns a new projectile from start to target.
 * @param {number} startX - Starting X coordinate.
 * @param {number} startY - Starting Y coordinate.
 * @param {number} targetX - Target X coordinate.
 * @param {number} targetY - Target Y coordinate.
...
// Assume 'particles' is a globally defined array.
const MAX_PARTICLES = 200; // From performance objective

/**
 * Spawns a particle at the given coordinates with a specified color.
 * Enforces a maximum particle limit.
